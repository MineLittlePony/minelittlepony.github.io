...
export function resize (img: HTMLImageElement | HTMLCanvasElement, sizeShift: number): HTMLCanvasElement {
  ...
  const src = createContext(img.width, img.height)

  src.drawImage(img, 0, 0)
  src.clearRect(0, 0, 4, 2)

  img = src.canvas
  // Code above is safe to comment to test the issue
  // Load SD skin and try to resize it
  // Then load HD skin and do the same to see the difference

  const width = 64 << sizeShift
  const height = img.width === img.height ? width : width / 2
